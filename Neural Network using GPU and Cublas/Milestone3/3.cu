...
{ /*copy training data to A[0]*/
  for(int i=0;i<nbatch;i++)
    ...
    //compute delta[L-1]
    matrix_add<<<1,1>>>(A[L-1], y + OUT_NUM *iter*nbatch, delta[L-1], -1, OUT_NUM * nbatch);
    //compute delta[L-2]
...
//gradient decent
    matrix_add<<<1,1>>>(W[L-1], Wu[L-1], W[L-1], -alpha/nbatch, OUT_NUM * nhidden);
    matrix_add<<<1,1>>>(B[L-1], Bu[L-1], B[L-1], -alpha/nbatch, OUT_NUM);
        
    //compute following hidden layer W and B
    for(int l = L-2; l>1;l--)
        { matrix_add<<<1,1>>>(W[l], Wu[l], W[l], -alpha/nbatch, nhidden * nhidden);
          matrix_add<<<1,1>>>(B[l], Bu[l], B[l], -alpha/nbatch, nhidden);
        }
    matrix_add<<<1,1>>>(W[1], Wu[1], W[1], -alpha/nbatch, SIZE * nhidden);
    matrix_add<<<1,1>>>(B[1], Bu[1], B[1], -alpha/nbatch, nhidden);
