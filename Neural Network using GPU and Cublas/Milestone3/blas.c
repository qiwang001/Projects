...
void initialize_y_test(int * rand_num, float *y, int* test_label)
{
...
//start to test
float * y_test = (float*) malloc(NUM_TEST*sizeof(float));
initialize_y(rand_num, y_test, test_label);
niter = NUM_TEST/nbatch;
int total = niter * nbatch;
int hit = 0;
for(int iter=0;iter<niter;iter++)
{ //copy training data to A[0]
  copy_training_data_to_A0(A[0],test_image, rand_num, nbatch, iter);
  // forward propagation
  cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nhidden, nbatch, SIZE, alpha_1, W[1], nhidden, A[0], SIZE, beta, Z[1], nhidden);
  activation_relu(Z[1], B[1], A[1], nhidden, nbatch);
    for(int l=2;l<L-1;l++)
    ...
    int j,correct,max=-9999999,start, end,guess;
    for(int i=0;i<nbatch;i++)
        {for(int j=0; j<OUT_NUM;j++)
          {
          max=-9999999;
          if(A[L-1][i*OUT_NUM+j] >max );
            {max =A[L-1][i*OUT_NUM+j];guess=j;}
