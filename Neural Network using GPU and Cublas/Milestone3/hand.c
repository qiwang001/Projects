...
void hand_mm(float* A, float*B, float *C, int m, int n, int k, float alpha, float beta)
{
    ...
    for(int i=0;i<m*n;i++)
        ...
        int row = i - col * m;
        for(int j=0;j<k;j++)
            sum +=  A[row + m * j] * B[col * k  + j];

        C[i] = C[i] * beta + alpha * sum;
        }

}
//transpose m by n matrix A, stored column major
void trans(float * A, int m, int n)
{
    int row, col;
...
{
//shuffle(rand_num);
initialize_y(rand_num, y, train_label);
...
{ //copy training data to A[0]
  ...
  hand_mm(W[1],A[0],Z[1], nhidden, nbatch, SIZE,1,0);
  activation_relu(Z[1], B[1], A[1], nhidden, nbatch);
    for(int l=2;l<L-1;l++)
    {
    hand_mm(W[l],A[l-1],Z[l],nhidden, nbatch, nhidden,1,0);
    ...
    hand_mm(W[L-1],A[L-2],Z[L-1],OUT_NUM, nbatch, nhidden,1,0);
    ...
    trans(W[L-1],OUT_NUM,nhidden);
    hand_mm(W[L-1],delta[L-1],delta[L-2],nhidden, nbatch, OUT_NUM,1,0);
    ...
    {trans(W[l+1], nhidden , nhidden );
   
    hand_mm(W[l+1],delta[l+1],delta[l],nhidden, nbatch,nhidden,1,0);
    }
    ...
    {
    trans(A[L-2] + i * nhidden, nhidden, 1);
    hand_mm(delta[L-1] + i * OUT_NUM,A[L-2] + i * nhidden,Wu[L-1],OUT_NUM, nhidden,1,1,1);
   ...
   //compute following hidden layer W and B
    ...
    trans(A[l-1], nhidden,nbatch  );
    
    hand_mm(delta[l],A[l-1],Wu[l],nhidden, nhidden, nbatch,1,0);
    //cblas_sgemm(CblasColMajor, CblasNoTrans,CblasNoTrans, OUT_NUM, 1, nbatch ,1, delta[l], OUT_NUM, ones_nbatch, nbatch, 0, Bu[l], OUT_NUM);
    hand_mm(delta[l],ones_nbatch,Bu[l],OUT_NUM, 1, nbatch,1,0);
    }
...
{   //cblas_sgemm(CblasColMajor, CblasNoTrans, CblasTrans, nhidden, SIZE,1,1,delta[1] + i * nhidden, nhidden, A[0] + i * SIZE, SIZE,1,Wu[1],nhidden);
    trans(A[0] + i * SIZE,SIZE,1);
    hand_mm(delta[1] + i * nhidden,A[0] + i * SIZE,Wu[1],nhidden, SIZE,1,1,1);
}
    //compute Bu[1]
    ...
    //compute following hidden layer W and B
    for(int l = L-2; l>1;l--)
        { matrix_add(W[l], Wu[l], W[l], (float)-alpha/nbatch, nhidden * nhidden);
          matrix_add(B[l], Bu[l], B[l], (float)-alpha/nbatch, nhidden);
        }
...
for(int i=0;i < temp;i++)
    ...
    //cblas_sgemm(CblasColMajor, CblasNoTrans,CblasNoTrans, nhidden, nbatch, SIZE, alpha_1, W[1], nhidden, ptr, SIZE, beta, Z[1], nhidden);
    hand_mm(W[1],ptr,Z[1],nhidden, nbatch, SIZE,1,0);
    activation_relu(Z[1], B[1], A[1], nhidden, nbatch);

    //compute middle hidden layer for L>3 case

    //cblas_sgemm(CblasColMajor, CblasNoTrans,CblasNoTrans, OUT_NUM, nbatch, nhidden, alpha_1, W[L-1], OUT_NUM, A[L-2], nhidden, beta, Z[L-1], OUT_NUM);
    hand_mm(W[L-1],A[L-2],Z[L-1],OUT_NUM, nbatch, nhidden,1,0);
    activation_softmax(Z[L-1],B[L-1],A[L-1],nbatch);
    ptr1 = y_valid + i * nbatch;
    calculate_loss(A[L-1], ptr1, loss, nbatch, epo);
    }  
