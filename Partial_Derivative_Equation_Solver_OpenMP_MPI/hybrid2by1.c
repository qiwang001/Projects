...
{
int stat, nprocs, mype,im1,ia1,jm1,i,j,ja1,NT;
double t1,t2,t3,t4,grind_rate,time_used,x,y,u,v;
int nt=atoi(argv[2]), N=atoi(argv[1]), M=2*N;
double **pt0,**pt1;
double **c = data_half_matrix(N);
double **c_new = data_half_matrix(N);
double **c1 = data_half_matrix(N);
double **c1_new = data_half_matrix(N);
...
for(int n=2;n <= NT;n++)
    ...
    #pragma omp parallel for default(none) shared(c,c_new,dx,dt,N,L) private(ja1,jm1,x,y,u,v) schedule(static) num_threads(nt)
...
for(int n=2;n <= NT;n++)
    ...
    #pragma omp parallel for default(none) shared(c1,c1_new,dt,L,dx,N) private(jm1,ja1,x,y,u,v)schedule(static) num_threads(nt)
    //#pragma omp parallel for default(private) schedule(static) num_threads(nt)
    for( int i=1;i<N/2-1; i++)
    {
        x= -L/2.0 + (i + N/2)*dx;
        v= -1.414 * x;
